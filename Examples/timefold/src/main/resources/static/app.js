...
function refreshTimeTable() {
  $.getJSON("/timeTable", function (timeTable) {
    ...
    const timeTableByspeaker = $("#timeTableByspeaker");
    timeTableByspeaker.children().remove();
    const timeTableBylanguage = $("#timeTableBylanguage");
    timeTableBylanguage.children().remove();
    ...
    const theadByspeaker = $("<thead>").appendTo(timeTableByspeaker);
    const headerRowByspeaker = $("<tr>").appendTo(theadByspeaker);
    headerRowByspeaker.append($("<th>Timeslot</th>"));
    const speakerList = [...new Set(timeTable.lessonList.map(lesson => lesson.speaker))];
    $.each(speakerList, (index, speaker) => {
      headerRowByspeaker
        .append($("<th/>")
          .append($("<span/>").text(speaker)));
    });
    const theadBylanguage = $("<thead>").appendTo(timeTableBylanguage);
    const headerRowBylanguage = $("<tr>").appendTo(theadBylanguage);
    headerRowBylanguage.append($("<th>Timeslot</th>"));
    const languageList = [...new Set(timeTable.lessonList.map(lesson => lesson.language))];
    $.each(languageList, (index, language) => {
      headerRowBylanguage
        .append($("<th/>")
    ...
    const tbodyByspeaker = $("<tbody>").appendTo(timeTableByspeaker);
    const tbodyBylanguage = $("<tbody>").appendTo(timeTableBylanguage);
      ...
      rowByspeaker
        .append($(`<th class="align-middle"/>`)
          .append($("<span/>").text(`
                ...
                `)));
      $.each(speakerList, (index, speaker) => {
        rowByspeaker.append($("<td/>").prop("id", `timeslot${timeslot.id}speaker${convertToId(speaker)}`));
      });

      const rowBylanguage = $("<tr>").appendTo(tbodyBylanguage);
      rowBylanguage
        .append($(`<th class="align-middle"/>`)
          .append($("<span/>").text(`
                    ${timeslot.dayOfWeek.charAt(0) + timeslot.dayOfWeek.slice(1).toLowerCase()}
                ...
                `)));
      $.each(languageList, (index, language) => {
        rowBylanguage.append($("<td/>").prop("id", `timeslot${timeslot.id}language${convertToId(language)}`));
      });
    });

    $.each(timeTable.lessonList, (index, lesson) => {
      const color = pickColor(lesson.title);
      const lessonElementWithoutDelete = $(`<div class="card" style="background-color: ${color}"/>`)
        .append($(`<div class="card-body p-2"/>`)
          .append($(`<h5 class="card-title mb-1"/>`).text(lesson.title))
          .append($(`<p class="card-text ml-2 mb-1"/>`)
            .append($(`<em/>`).text(`by ${lesson.speaker}`)))
          .append($(`<small class="ml-2 mt-1 card-text text-muted align-bottom float-right"/>`).text(lesson.id))
          .append($(`<p class="card-text ml-2"/>`).text(lesson.language)));
      ...
      if (lesson.timeslot == null || lesson.room == null) {
        ...
        $(`#timeslot${lesson.timeslot.id}speaker${convertToId(lesson.speaker)}`).append(lessonElementWithoutDelete.clone());
        $(`#timeslot${lesson.timeslot.id}language${convertToId(lesson.language)}`).append(lessonElementWithoutDelete.clone());
      }
    });
  });
}
...
function addLesson() {
  var title = $("#lesson_title").val().trim();
  $.post("/lessons", JSON.stringify({
    "title": title,
    "speaker": $("#lesson_speaker").val().trim(),
    "language": $("#lesson_language").val().trim()
  }), function () {
    refreshTimeTable();
  }).fail(function (xhr, ajaxOptions, thrownError) {
    showError("Adding lesson (" + title + ") failed.", xhr);
  });
